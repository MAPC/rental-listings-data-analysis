...
# Load packages
install.packages("pacman", repos = "http://cran.us.r-project.org") # pacman package allows for easy installing and loading of multiple packages in a single function
library(pacman)
pacman::p_load(
  plyr,
  dplyr,
...
#categorize Quarter
colnames(listings_unique_temp)[which(names(listings_unique_temp) == "Year")] <-
  "year"
...
listings_unique_temp$update_lag <-
  as.Date(listings_unique_temp$updated_date) - as.Date(listings_unique_temp$post_date)

#replace lat long attributes with GEOLOCATED lat long
listings_unique_temp$latitude_merge[is.na(listings_unique_temp$latitude_merge)] <-
  0
listings_unique_temp$latitude_original <-
...
listings_unique_temp$latitude <-
  ifelse(
    listings_unique_temp$latitude_merge < 0 &
      listings_unique_temp$source_id == 2,
    listings_unique_temp$latitude_merge,
    listings_unique_temp$latitude
  )

listings_unique_temp$longitude_merge[is.na(listings_unique_temp$longitude_merge)] <-
  0
listings_unique_temp$longitude_original <-
  listings_unique_temp$longitude

listings_unique_temp$longitude <-
  ifelse(
    listings_unique_temp$longitude_merge < 0 &
      listings_unique_temp$source_id == 2,
...
event.Points <-
  SpatialPoints(
    data.frame(
      latitude = listings_unique_temp$latitude,
      longitude = listings_unique_temp$longitude
    ),
...
pnt_parcels_consortium <-
  over(event.Points, parcels_consortium_abcd)

#extract the names from each overlay
try (if ("parloc_id" %in% colnames(pnt_parcels_consortium))
  parloc_id <- pnt_parcels_consortium$parloc_id
  else
    stop("Error! No town attribute detected in town shapefile!"))

try (if ("yr_built" %in% colnames(pnt_parcels_consortium))
  yr_built <- pnt_parcels_consortium$yr_built
  else
    stop("Error! No town attribute detected in town shapefile!"))

try (if ("luc_adj_1" %in% colnames(pnt_parcels_consortium))
  luc_adj_1 <- pnt_parcels_consortium$luc_adj_1
  else
    stop("Error! No town attribute detected in town shapefile!"))

try (if ("luc_adj_2" %in% colnames(pnt_parcels_consortium))
...
zip.shape <-
  readOGR(dsn = path.expand(spatialSrcPath), layer = "zip_MA")
...
# add the parcel attributes to the listings
listings_unique <-
  cbind(listings_unique_w_parcel,
        pnt_zip.shape$postcode,
        pnt_zip.shape$pc_name)
...
listings_unique$roomrent <-
  grepl(
    ...
    listings_unique$title
  )
listings_unique$sublet <- grepl(" SUBLET ", listings_unique$title)
listings_unique$shortterm <-
  grepl(" SHORT TERM ", listings_unique$title)
listings_unique$shared <-
  grepl(" SHARED | SHARE ", listings_unique$title)
...
listings_unique_units_clean <- listings_unique_units %>%
  filter(!(
    ...
    numRooms != -1) %>% # also filters out all listings with numRooms == -1
...
listings_unique_units_boston <-
  filter(listings_unique_units_clean, muni == "BOSTON")
listings_unique_units_cambridge <-
  filter(listings_unique_units_clean, muni == "CAMBRIDGE")
listings_unique_units_quincy <-
  filter(listings_unique_units_clean, muni == "QUINCY")
listings_unique_units_somerville <-
  filter(listings_unique_units_clean, muni == "SOMERVILLE")
listings_unique_units_arlington <-
  filter(listings_unique_units_clean, muni == "ARLINGTON")


###################################### STEP 7: ANNUAL SUMMARY STATISTICS ######################################

listings_summary_full_annual <-
  ddply(
    listings_unique_units_clean,
    c("numRooms", "year"),
    summarise,
...
listings_summary_muni_annual <-
  ddply(
    listings_unique_units_clean,
    c("muni", "numRooms", "year"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_ct_annual <-
  ddply(
    listings_unique_units_clean,
    c("ct10_id", "numRooms", "year"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_nhood_annual <-
  ddply(
    listings_unique_units_clean,
    c("neighborhood_01", "muni", "numRooms", "year"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_zip_annual <-
  ddply(
    listings_unique_units_clean,
    c("zip_code", "muni", "numRooms", "year"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_boston_annual <-
  filter(listings_summary_muni_annual, muni == "BOSTON")
listings_summary_cambridge_annual <-
  filter(listings_summary_muni_annual, muni == "CAMBRIDGE")
listings_summary_quincy_annual <-
  filter(listings_summary_muni_annual, muni == "QUINCY")
listings_summary_somerville_annual <-
  filter(listings_summary_muni_annual, muni == "SOMERVILLE")
listings_summary_arlington_annual <-
  filter(listings_summary_muni_annual, muni == "ARLINGTON")

listings_summary_boston_nhood_annual <-
  filter(listings_summary_nhood_annual, muni == "BOSTON")
listings_summary_cambridge_nhood_annual <-
  filter(listings_summary_nhood_annual, muni == "CAMBRIDGE")
listings_summary_quincy_nhood_annual <-
  filter(listings_summary_nhood_annual, muni == "QUINCY")
listings_summary_somerville_nhood_annual <-
  filter(listings_summary_nhood_annual, muni == "SOMERVILLE")
listings_summary_arlington_nhood_annual <-
  filter(listings_summary_nhood_annual, muni == "ARLINGTON")

listings_summary_full_annual_age <-
  ddply(
    listings_unique_units_clean,
    c("source_id", "numRooms", "year", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_muni_annual_age <-
  ddply(
    listings_unique_units_clean,
    c("source_id", "muni", "ct10_id", "numRooms", "year", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_nhood_annual_age <-
  ddply(
    listings_unique_units_clean,
    c(
      "source_id",
...
listings_summary_zip_annual_age <-
  ddply(
    listings_unique_units_clean,
    c(
      "source_id",
...
listings_summary_boston_annual_age <-
  filter(listings_summary_muni_annual_age, muni == "BOSTON")
listings_summary_cambridge_annual_age <-
  filter(listings_summary_muni_annual_age, muni == "CAMBRIDGE")
listings_summary_quincy_annual_age <-
  filter(listings_summary_muni_annual_age, muni == "QUINCY")
listings_summary_somerville_annual_age <-
  filter(listings_summary_muni_annual_age, muni == "SOMERVILLE")
listings_summary_arlington_annual_age <-
  filter(listings_summary_muni_annual_age, muni == "ARLINGTON")


listings_summary_boston_nhood_annual_age <-
  filter(listings_summary_nhood_annual_age, muni == "BOSTON")
listings_summary_cambridge_nhood_annual_age <-
  filter(listings_summary_nhood_annual_age, muni == "CAMBRIDGE")
listings_summary_quincy_nhood_annual_age <-
  filter(listings_summary_nhood_annual_age, muni == "QUINCY")
listings_summary_somerville_nhood_annual_age <-
  filter(listings_summary_nhood_annual_age, muni == "SOMERVILLE")
listings_summary_arlington_nhood_annual_age <-
  filter(listings_summary_nhood_annual_age, muni == "ARLINGTON")

listings_unique_units_fwd <-
  filter(listings_unique_units_clean, fwd_geolocated == "TRUE")
listings_summary_full_annual_age_fwd <-
  ddply(
    listings_unique_units_fwd,
    c("numRooms", "year", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_nhood_quarterly <-
  ddply(
    listings_unique_units_clean,
    c("neighborhood_01", "muni", "numRooms", "quarter"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_ct_quarterly <-
  ddply(
    listings_unique_units_clean,
    c("ct10_id", "muni", "numRooms", "quarter"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_zip_quarterly <-
  ddply(
    listings_unique_units_clean,
    c("zip_code", "muni", "numRooms", "quarter"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_boston_quarterly <-
  filter(listings_summary_muni_quarterly, muni == "BOSTON")
listings_summary_cambridge_quarterly <-
  filter(listings_summary_muni_quarterly, muni == "CAMBRIDGE")
listings_summary_quincy_quarterly <-
  filter(listings_summary_muni_quarterly, muni == "QUINCY")
listings_summary_somerville_quarterly <-
  filter(listings_summary_muni_quarterly, muni == "SOMERVILLE")
listings_summary_arlington_quarterly <-
  filter(listings_summary_muni_quarterly, muni == "ARLINGTON")

listings_summary_boston_nhood_quarterly <-
  filter(listings_summary_nhood_quarterly, muni == "BOSTON")
listings_summary_cambridge_nhood_quarterly <-
  filter(listings_summary_nhood_quarterly, muni == "CAMBRIDGE")
listings_summary_quincy_nhood_quarterly <-
  filter(listings_summary_nhood_quarterly, muni == "QUINCY")
listings_summary_somerville_nhood_quarterly <-
  filter(listings_summary_nhood_quarterly, muni == "SOMERVILLE")
listings_summary_arlington_nhood_quarterly <-
  filter(listings_summary_nhood_quarterly, muni == "ARLINGTON")

listings_summary_full_quarterly_age <-
  ddply(
    listings_unique_units_clean,
    c("source_id", "numRooms", "quarter", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_muni_quarterly_age <-
  ddply(
    listings_unique_units_clean,
    c("source_id", "muni", "numRooms", "quarter", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_ct_quarterly_age <-
  ddply(
    listings_unique_units_clean,
    c("source_id", "ct10_id", "numRooms", "quarter", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_zip_quarterly_age <-
  ddply(
    listings_unique_units_clean,
    c("source_id", "zip_code", "numRooms", "quarter", "periodblt"),
    summarise,
    rentcount = length(numRooms),
    meanrent = round(mean(ask), 0),
    medrent = round(median(ask), 0)
...
listings_summary_boston_quarterly_age <-
  filter(listings_summary_muni_quarterly_age, muni == "BOSTON")
listings_summary_cambridge_quarterly_age <-
  filter(listings_summary_muni_quarterly_age, muni == "CAMBRIDGE")
listings_summary_quincy_quarterly_age <-
  filter(listings_summary_muni_quarterly_age, muni == "QUINCY")
listings_summary_somerville_quarterly_age <-
  filter(listings_summary_muni_quarterly_age, muni == "SOMERVILLE")
listings_summary_arlington_quarterly_age <-
  filter(listings_summary_muni_quarterly_age, muni == "ARLINGTON")

listings_summary_boston_nhood_quarterly_age <-
  filter(listings_summary_nhood_quarterly_age, muni == "BOSTON")
listings_summary_cambridge_nhood_quarterly_age <-
  filter(listings_summary_nhood_quarterly_age, muni == "CAMBRIDGE")
listings_summary_quincy_nhood_quarterly_age <-
  filter(listings_summary_nhood_quarterly_age, muni == "QUINCY")
listings_summary_somerville_nhood_quarterly_age <-
  filter(listings_summary_nhood_quarterly_age, muni == "SOMERVILLE")
listings_summary_arlington_nhood_quarterly_age <-
...
write.csv(listings_summary_counts,
          ...
          row.names = FALSE)
write.csv(listings_unique_units,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_unique_units_clean,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_unique_units_boston,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_unique_units_cambridge,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_unique_units_quincy,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_unique_units_somerville,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_unique_units_arlington,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_full_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_muni_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_ct_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_nhood_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_zip_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_boston_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_cambridge_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_quincy_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_somerville_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_arlington_annual,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_boston_nhood_annual,
          here(
            "data",
            "finished",
            paste(
...
write.csv(listings_summary_quincy_nhood_annual,
          here(
            "data",
            "finished",
            paste(
...
write.csv(listings_summary_full_annual_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_muni_annual_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_nhood_annual_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_zip_annual_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_boston_annual_age,
          here(
            "data",
            "finished",
            paste(
...
write.csv(listings_summary_quincy_annual_age,
          here(
            "data",
            "finished",
            paste(
...
write.csv(listings_summary_full_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_muni_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_ct_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_nhood_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_zip_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_boston_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_cambridge_quarterly,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_quincy_quarterly,
          here(
            "data",
            "finished",
            paste(
...
write.csv(listings_summary_arlington_quarterly,
          here(
            "data",
            "finished",
            paste(
...
write.csv(listings_summary_full_quarterly_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_muni_quarterly_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_ct_quarterly_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_nhood_quarterly_age,
          here(
            "data",
            "finished",
            paste(
          ...
          row.names = FALSE)
write.csv(listings_summary_zip_quarterly_age,
          here(
            "data",
            "finished",
            paste(
